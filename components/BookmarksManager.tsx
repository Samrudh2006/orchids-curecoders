...
export default function BookmarksManager({ isOpen, onClose }: BookmarksManagerProps) {
  ...
  const removeBookmark = (id: string) => {
    fetch(`${getApiUrl()}/api/bookmarks/${id}`, { 
      method: 'DELETE',
      headers: getAuthHeaders()
    })
      .then(res => {
  ...
  const addTag = (id: string, tag: string) => {
    ...
    if (!bookmark) return;
    const updatedTags = [...new Set([...bookmark.tags, tag.trim()])];
    
    fetch(`${getApiUrl()}/api/bookmarks/${id}/tags`, {
      method: 'PUT',
      headers: { 
      ...
      <div className="w-full max-w-2xl max-h-[80vh] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        ...
        {!user ? (
          <div className="flex-1 overflow-auto p-8 flex items-center justify-center">
            <LockedFeatureGate featureName="Bookmarks Manager" />
          </div>
        ) : (
          <>
